...
**Status:** Note date from photo EXIF **merged to `main`** (6-task plan, 223 tests passing). Note card location display fix on branch `fix/note-card-location-display` (committed, not yet merged). Prior: Blog generation reliability fixes merged 2026-06-10 (commit `fab6e91`). Prior: "View Blog" button merged 2026-06-09 (commit `fdcf2c8`). Prior: Background photo upload + offline photo capture merged 2026-06-05. **Next backlog item:** Move notes between trips (Priority 3).
...
**Note card location display — fix done, bigger feature designed (NOT YET IMPLEMENTED — pick up in new thread):**

1. *Duplicate location display fix (DONE, on branch `fix/note-card-location-display`, not merged):* `NoteCard.tsx` showed location twice — `note.city` top-right next to the timestamp, and `note.place_name` below the note text with 📍. Removed `city` (and `pending.city`) from the header meta line; header now shows only the relative time. `place_name` remains the single location indicator below the text. tsc clean, 223/223 tests pass.

2. *Designed but not implemented — smarter `place_name` resolution + backfill:* Investigation found `city` is GPS-derived (reverse-geocoded from device/EXIF coordinates) but `place_name` was previously **user-typed text only** (null unless manually edited). Agreed design for a follow-up:
   - Add `reverseGeocodePlace(lat, lng)` to `src/services/locationService.ts`: one `reverseGeocodeAsync` call returning `{ city, placeName }`, where `city = city ?? subregion ?? region` and `placeName = name ?? street ?? city ?? subregion ?? region` (always at least as specific as city, often a POI/street).
   - In `NoteCaptureSheet.tsx`: replace the EXIF-coords reverse-geocode effect (currently only sets `exifCity`) with one that also captures `placeName`; add `placeName` to `LocationFix` (returned from `getCurrentLocation()` in `locationService.ts`); in `handleSave`, the `auto` patch passed to `resolveLocationEdit` sets `place_name: autoPlaceName` (was hardcoded `null`), falling back EXIF → live GPS like `autoCity`/`autoLat`/`autoLng` already do. Manual edits (`wasEdited`) are unchanged — user text still wins.
   - **Backfill (~20 existing notes):** automatic, on app launch, gated by an `AsyncStorage` flag-equivalent — actually: query notes for the current user where `place_name IS NULL AND lat IS NOT NULL AND lng IS NOT NULL`, process in small batches (~5) with a short delay via `reverseGeocodePlace`, update each note's `place_name` (and `city` if null) via `noteService`. Re-running the query naturally skips already-backfilled notes, so it's safely resumable across launches without a "done" flag if preferred — only mark fully done once the query returns zero rows. No new UI/screen, no new API keys (uses existing on-device `expo-location`).
   - No DB migration needed — `place_name` column already exists.
   - **Next steps for new thread:** write a short spec (or go straight to a plan, given how concrete this already is) and implement via subagent-driven-development, same as the EXIF-date feature.

**Itinerary creation from blog (DESIGN DIRECTION CONFIRMED, NOT STARTED):** User confirmed the existing post-MVP plan in `docs/superpowers/specs/2026-05-28-phase-9-blog-generation-design.md` ("Post-MVP Feature — Itinerary Creation", lines ~136–139) matches what they want: once a trip has ≥3 days with timestamped (`occurred_at`, now available) + geolocated notes, `generate-blog` produces a day-by-day itinerary as a structured block, stored in a new `itinerary jsonb` column on `blog_posts`, rendered as a conditional section/tab on `BlogPostScreen` (hidden if absent/sparse). **Next steps for new thread:** write a full spec for this (it's bigger than the location fix — touches the edge function prompt, a migration, and new UI), then a plan.
...
| **Move notes between trips** | No way to re-assign a note to a different trip if captured under the wrong one. | Add "Move to trip…" action in `NoteEditSheet` — `TripSelector`-style picker, `updateNote` with new `trip_id`. Migration needed to allow `UPDATE` on `trip_id`. |
