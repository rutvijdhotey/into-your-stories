...
**Status:** Trip cover photo (banner image) implemented on branch `feature/trip-cover-photo` — 195 tests passing, tsc clean; awaiting merge. Prior: QA #2 editable note location (PR #8) + dead `PhotoGrid` removal (PR #9) merged to `main` 2026-05-30. Phase 9 (Blog generation) merged + pushed 2026-05-29. **Next backlog item:** TBD (see backlog table).

**Trip cover photo (done, pending merge):** User picks + crops a photo from the device library to set the trip banner; tap the camera icon for a Choose/Remove menu. New units: `useCoverPhoto` hook (pick→upload→save / remove + busy), `photoService.uploadCoverPhoto` (stores at `userId/trip-covers/tripId.jpg` with a `?v=` cache-buster), `tripService.updateCoverPhoto`, shared `photoHelpers.ensureMediaLibraryPermission`. Banner refresh is free via `useTripDetail`'s realtime subscription. **Known constraint:** iOS `expo-image-picker` `allowsEditing` only crops square (ignores custom aspect), so the user frames a square and the banner center-crops it via `resizeMode="cover"`; true banner-ratio crop would need `expo-image-manipulator` (out of scope). **Follow-up idea:** on iOS, swap the `Alert.alert` cover menu for `ActionSheetIOS` (matches HomeScreen's idiom) — non-blocking polish.
