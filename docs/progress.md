...
**Status:** Priority 1 backlog fixes (voice dictation append + autocorrect) **merged to `main` 2026-06-04** — 197 tests passing, tsc clean. Prior: Trip cover photo merged + pushed 2026-06-04. **Next backlog item:** Background photo upload + offline photo capture (Priority 2 — spec written, implementation plan in progress).
...
| **Background photo upload + offline photo capture** (Priority 2) | Photos upload synchronously before the note row saves, blocking the UI. Offline capture with photos is impossible. | Spec written: `docs/superpowers/specs/2026-06-04-background-photo-upload-design.md`. Two-queue approach: `photoUploadQueue` alongside `offlineQueue`; notes save instantly, photos drain in background. Implementation plan in progress. |
